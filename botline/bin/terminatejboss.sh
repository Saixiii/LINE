...
JBOSS_STOP="/host=${JBOSS_NODE}/server-config=${JBOSS_SERVER} :kill"
...
then
   ...
   do
      ...
      if func_CheckStop
      then
         func_LINE "TerminateJboss:\nStop complete on nodename=${JBOSS_NODE}, servername=${JBOSS_SERVER} maxwaitseconds=${WAIT_TIMEOUT} elapsedtimeseconds=${TIME_MIN}"
         break
      fi
      if [ ${TIME_MIN} -gt ${WAIT_TIMEOUT} ]
      then
         func_LINE "TerminateJboss:\nWaiting time is over to stop on nodename=${JBOSS_NODE}, servername=${JBOSS_SERVER} maxwaitseconds=${WAIT_TIMEOUT} elapsedtimeseconds=${TIME_MIN}"
         break
      fi
      func_LINE "TerminateJboss:\nWaiting ${TIME_MIN} of ${WAIT_TIMEOUT} seconds for ${JBOSS_SERVER} to stop."
   done
else
   func_LINE "TerminateJboss:\nFail to excetued stop command on nodename=${JBOSS_NODE}, servername=${JBOSS_SERVER}"
fi
