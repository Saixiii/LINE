...
JBOSS_RESTART="/host=${JBOSS_NODE}/server-config=${JBOSS_SERVER} :restart"
...
#-------------------------------------------------------------------------------
# Function    : func_Restartjboss
# Description : Start jboss server
# Parameters  : Message
# Return      : <None>
#-------------------------------------------------------------------------------
func_Restartjboss() {
   APP_START=`${JBOSS_SCRIPT} --controller=${JBOSS_IP}:${JBOSS_PORT} --connect --user=${JBOSS_USER} --password=${JBOSS_PASS} --timeout=${JBOSS_TIMEOUT} "${JBOSS_RESTART}"`
   CMD_RESULT=`echo "${APP_START}" |grep outcome |awk -F'"' '{print $4}'`
   if [ ${CMD_RESULT} == "success" ]
   then
      # 0 = true
...
if func_CheckStop
then
   func_LINE "RestartJboss:\nServer has been stopped on nodename=${JBOSS_NODE}, servername=${JBOSS_SERVER}"
elif func_Restartjboss
then
   while :
   do
      sleep ${WAIT_INTERVAL}
      ...
      if func_CheckStart
      then
         func_LINE "RestartJboss:\nRestart complete on nodename=${JBOSS_NODE}, servername=${JBOSS_SERVER} maxwaitseconds=${WAIT_TIMEOUT} elapsedtimeseconds=${TIME_MIN}"
         break
      fi
   ...
   func_LINE "RestartJboss:\nFail to excetued restart command on nodename=${JBOSS_NODE}, servername=${JBOSS_SERVER}"
fi
